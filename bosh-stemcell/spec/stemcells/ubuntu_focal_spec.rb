...
describe 'Ubuntu 20.04 stemcell image', stemcell_image: true do
...
HERE
  ...
  describe 'installed packages' do
    ...
    let(:dpkg_list_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal.txt')).map(&:chop) }
    let(:dpkg_list_google_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal-google-additions.txt')).map(&:chop) }
    let(:dpkg_list_vsphere_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal-vsphere-additions.txt')).map(&:chop) }
    let(:dpkg_list_azure_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal-azure-additions.txt')).map(&:chop) }
    let(:dpkg_list_cloudstack_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal-cloudstack-additions.txt')).map(&:chop) }
    let(:dpkg_list_softlayer_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-focal-softlayer-additions.txt')).map(&:chop) }
